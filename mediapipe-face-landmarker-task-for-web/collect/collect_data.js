...
async function faceleClick(event) {
    ...
    if (faceLandmarkerResult.faceLandmarks && faceLandmarkerResult.faceLandmarks.length > 0) {
        const flattenedCoordinates = flattenCoordinates(faceLandmarkerResult.faceLandmarks[0]);
...
function saveData() {
    ...
    faceSignData.push({
        pose: results.faceLandmarks.map(flattenCoordinates)[0],
        label: selectedFaceSign
    });
}

function exportData() {
    const dataStr = JSON.stringify(faceSignData, null, 2);
    const dataUri = 'data:application/json;charset=utf-8,' + encodeURIComponent(dataStr);
    const exportFileDefaultName = 'facesign_data.json';
...
document.getElementById('facesignSelect').addEventListener('change', function(event) {
    selectedFaceSign = event.target.value;
});
